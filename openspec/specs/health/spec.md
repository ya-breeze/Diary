...
The server runs health checks automatically in the background.
...
Users can see the current health status of their data.
...
Users can trigger automatic fixes for fixable issues.
...
Individual orphaned files can be deleted, attached to an entry, or ignored.
...
Every orphan mutation triggers a fresh orphan scan so the returned state is always current.
