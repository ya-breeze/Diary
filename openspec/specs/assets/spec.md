...
Files can be added to an entry via drag-and-drop or the file browser.
...
The server enforces limits and extension rules before saving files.
...
The editor shows attached images and allows removal.
...
Assets are served at `GET /v1/assets/{path}`, scoped to the requesting family's directory.
