...
The server creates a compressed backup archive automatically.
...
Each backup is a single `.tar.gz` file containing the database and all assets.
...
The backup task is idempotent within a single day.
...
Archives are written atomically to avoid corrupt partial files.
...
Old backups are pruned to stay within a configured maximum count.
