...
## Purpose
The family's tag vocabulary as a managed resource: usage counts (`GET /v1/tags/stats`), a dedicated Tags page for listing/browsing tags, and cross-entry rename (merge-on-collision) and delete via `PATCH`/`DELETE /v1/tags/{name}`.
## Requirements
