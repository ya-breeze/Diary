...
## Purpose
How users authenticate: login, session persistence across reloads, silent token refresh, logout, and protection of routes.

## Requirements

### Requirement: Login with email and password
A user SHALL log in with their email and password; on success they SHALL be redirected to the diary.
...
Auth state SHALL survive a browser refresh.
...
The access token is short-lived; the refresh token SHALL be used to issue a new one silently.
...
All routes except `/login` and `/api/*` SHALL require authentication.
