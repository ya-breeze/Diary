...
- [ ] 2.1 `SuggestTags` (`gemini.go`): wrap `GenerateContent` in a bounded retry loop that retries on a model-provider 5xx or 429 (detect via the genai API-error type/`Code`), ~3 attempts with a short backoff (~200ms→~400ms + jitter)
- [ ] 2.2 For a 429, honor the `Retry-After` hint when present but cap the wait at the remaining retry budget and `ctx` deadline (fail fast if the hint exceeds the budget); no hint → standard 5xx backoff
- [ ] 2.3 Abort the retry loop early on `ctx` cancellation/deadline; on exhausting attempts return the wrapped error; do not retry other 4xx or empty-but-200 responses
- [ ] 2.4 Add a helper to classify a genai error as retryable (5xx/429) vs not, guarding for unknown error shapes (treat unknown as non-retryable)
...
- [ ] 6.3 `backend/pkg/ai`: test the retry classifier (5xx/429 retryable, other 4xx/unknown not); a transient 5xx-then-success returns suggestions, a persistent transient error returns an error, and a 429 with `Retry-After` waits (bounded) before retrying
