...
- **Retry the model call on transient 5xx errors** from the Gemini API (a bounded number of attempts with short backoff, respecting the request context) before giving up. Persisting failures still surface as an error (which the frontend now toasts). Empty 2xx responses are **not** retried.
...
- `ai-tagging`: extends graceful-degradation behavior — an empty or blocked model response SHALL degrade to "no suggestions" rather than surfacing an error, the reason SHALL be logged, and transient 5xx errors from the model provider SHALL be retried (bounded) before the request fails.
